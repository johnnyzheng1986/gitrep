 ...
 */
/**
* Alter by Qiujian 2012-04-24
* changeLog 	:
*			1. change the params set to destroyUrl : row.id(before) --> row (now)
			2.change the message-text for customer 
*/
(function($){
				...
				$.messager.confirm(opts.destroyMsg.confirm.title,opts.destroyMsg.confirm.msg,function(r){
					if (r){
						...
						if (row.isNewRecord){
							...
							if (opts.destroyUrl){
								//alter by Qiujian 2012-04-23 
								//原本是传递row.id，现改为传递整个row
								$.post(opts.destroyUrl,row, function(){
									if (opts.tree){
										dg.datagrid('reload');
										var t = $(opts.tree);
										var node = t.tree('find', row.id);
